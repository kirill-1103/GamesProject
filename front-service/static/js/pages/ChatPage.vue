			...
			<div class="card card3">
				<div class="row g-0">
					<div class="col-12 col-lg-5 col-xl-3 border-right">
						...
						<div v-if="dialogsIsLoaded" class="players">
							<div
								...
								class="companion"
								v-bind:class="{
									selected: companion && dialog.companion.id == companion.id,
								}"
							>
								<a
								...
								>
									...
									<div class="d-flex align-items-start">
										<img
											v-if="dialog.companion.photo && dialog.companion.imageUrl"
											:src="dialog.companion.imageUrl"
										...
										<div
											v-else-if="
												dialog.companion.photo && !dialog.companion.imageUrl
											"
											style="margin-top:50%;"
										...
										<img
											v-else-if="!dialog.companion.photo"
											src="../../img/default.png"
											class="rounded-circle mr-1"
											alt="img"
											width="65"
										...
										<div class="flex-grow-1 ml-3" style="margin-left: 10px;">
											...
											<div v-if="isOnline(dialog.companion.id)" class="small">
						...
						<div
							v-else="dialog.companion.photo && !dialog.companion.imageUrl"
					...
					<div class="col-12 col-lg-7 col-xl-9">
						<div
							v-if="companion != null"
							class="py-2 px-4 border-bottom d-none d-lg-block companion-div"
						...
						>
							...
							>
								<div class="position-relative">
									<img
										v-if="companion.photo && companion.imageUrl"
									...
									/>
									<div
										v-else-if="companion.photo && !companion.imageUrl"
										style="margin-top:50%;"
										class="spinner-border text-primary"
										role="status"
									>
						...
						<div class="position-relative" style="height: 100%">
							<div
								v-show="companion != null"
								class="chat-messages p-4"
								ref="chat"
							>
								<button
									v-if="unreadMessageInCurrentDialog"
									class="scroll-button"
								...
								<div
									...
									v-bind:class="{
										'chat-message-right': message.sender.id !== companion.id,
										'mb-4': message.sender.id !== companion.id,
										'pb-4': message.sender.id === companion.id,
								...
								>
									<div>
										...
										<div class="text-muted small text-nowrap mt-2">
											{{ getStringDate(message.sendingTime)[0] }}
										</div>
										<div class="text-muted small text-nowrap mt-2">
											{{ getStringDate(message.sendingTime)[1] }}
										</div>
									</div>
									<div class="flex-shrink-1 bg-light rounded py-2 px-3 mr-3">
										<div
											v-if="message.sender.id === player.id"
											class="font-weight-bold mb-1 loginInMessage"
										>
											You
										</div>
										<div v-else class="font-weight-bold mb-1 loginInMessage">
...
export default {
	...
	methods: {
			...
			axios
				...
				.then(res => {
					for (let dialog of res.data) {
						this.addDialog(
							this.getCompanionFromDialog(dialog),
							[],
							this.dialogHasUnreadMessage(dialog),
							dialog.lastMessage.sendingTime
						)
					}
					this.sortDialogs()
		...
		getCompanionFromDialog(dialog) {
			return dialog.player1.id === this.player.id
				? dialog.player2
				: dialog.player1
		},
		dialogHasUnreadMessage(dialog) {
			return (
				dialog.lastMessage.sender.id !== this.player.id &&
				dialog.lastMessage.readingTime == null
			)
		},
		...
		getStringDate(dateArray) {
			if (typeof dateArray === 'string') {
				...
				return [
					dateArray.substring(8, 10) +
						'.' +
						dateArray.substring(5, 7) +
						'.' +
						dateArray.substring(0, 4),
					dateArray.substring(11, 19),
				]
			}
		...
		addNewMessages(messages) {
			...
			for (let message of messages) {
				...
				this.setSenderAndRecipient(message).then(() => {
					let fromCurrentCompanion =
						this.companion && this.companion.id == message.senderId
					let toCurrentCompanion =
						this.companion && this.companion.id == message.recipientId
					let added = false
					for (let dialog of this.dialogs) {
			...
			let interval = setInterval(() => {
				...
				if (chat) {
					chat.addEventListener('scroll', event => {
						if (
							Math.abs(chat.scrollHeight - chat.scrollTop - chat.offsetHeight) <
							10
						) {
							this.unreadMessageInCurrentDialog = false
						}
					...
					axios
						...
						.then(res => {
							if (res.data.error) {
								...
								let dialogByCompanion = this.getDialogByCompanionIfExists(
									dialogCompanion.id
								)
								console.log(dialogCompanion)
									...
									if (dialogCompanion.photo) {
										axios
											...
											.then(res => {
												...
												this.openDialog(
													this.getDialogByCompanionIfExists(dialogCompanion.id)
												)
												this.sortDialogs()
											})
											.catch(err => {
		...
		goToPlayerPage() {
			this.$router.push('/player/' + this.companion.id)
		},
	},
	watch: {

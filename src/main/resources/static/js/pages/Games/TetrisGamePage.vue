<template>
  ...
  <StartGameComponent v-if="(!$store.state.playerGameId || $store.state.playerGameId === -1)" />
  <div class="container" v-else>
    <GameComponent :player="player" :entity="entity" :game="game"></GameComponent>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    updateGameFromDb() {
      axios.get("/api/tetris_game/processing/" + this.$store.state.playerGameId).then((response) => {
